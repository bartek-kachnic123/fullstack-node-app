...
function addCard(data_array, book) {
    ...
    data_array.push(`<p class="card-text text-end px-1">Data publikacji: <small>${formatDate(book.publishedDate)}</small></p>`);
    data_array.push(`<h5 class="card-text text-center text-uppercase py-5">${book.author}</h5>
                </div>
            </div>`);
